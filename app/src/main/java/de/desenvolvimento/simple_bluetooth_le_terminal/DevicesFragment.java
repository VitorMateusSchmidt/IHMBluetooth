...
public class DevicesFragment extends ListFragment {
    ...
    public void onResume() {
        ...
        if(bluetoothAdapter == null) {
            setEmptyText("<bluetooth LE não suportado>");
        } else if(!bluetoothAdapter.isEnabled()) {
            setEmptyText("<bluetooth está desativado>");
            if (menu != null) {
                listItems.clear();
            ...
            setEmptyText("<use SCAN para atualizar dispositivos>");
            if (menu != null)
    ...
    private void stopScan() {
        if(scanState == ScanState.NONE)
            return;
        setEmptyText("<Não foram encontrados dispositivos bluetooth>");
        if(menu != null) {
